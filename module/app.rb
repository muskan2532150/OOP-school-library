...
class App
  ...
  def store_all_data
    ...
    @person.each do |pk|
      if pk.is_a(Student)
        person_data.push({ 'Age' => pk.age, 'Classroom' => pk.Classroom, 'Name' => pk.name,
                           'Has Parent Permission' => pk.parent_permission })
      end
      if pk.is_a(Teacher)
        person_data.push({ 'Age' => pk.age, 'Specialization' => pk.specialization, 'Name' => pk.name,
                           'Has Parent Permission' => pk.parent_permission })
      end
    end
  ...
  def read_all_data
    ...
    pk.each do |person|
      if person.key?('Classroom')
        @person.push(Student.new(person['name'], person['age'], person['parent_permission'],
                                 person['classroom']))
      end
      if person.key?('specialization')
        @person.push(Teacher.new(person['name'], person['age'], person['parent_permission'],
